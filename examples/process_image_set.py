...
max_num_stars_to_process = (
    10  # maximum number of centroids to attempt to match per image
)
...
data_path = "/home/rick/Documenti/COTS-Star-Tracker/data"  # full path to your data
image_path = (
    "/home/rick/Documenti/COTS-Star-Tracker/star_images"  # full path to your images
)
cam_config_file_path = "/home/rick/Documenti/COTS-Star-Tracker/data/cam_config/camera_cal_1280_720.json"  # full path (including filename) of your cam config file
...
if darkframe_file_path is not None:
    if not os.path.exists(darkframe_file_path):
        ...
        print("darkframe file: " + darkframe_file_path)
else:
    print("no darkframe file provided, proceeding without one...")
...
indexed_star_pairs = np.load(
    os.path.join(data_path, cat_prefix + "indexed_star_pairs.npy")
)
...
for image_filename in image_names:
    ...
    q_est, idmatch, nmatches, x_obs, rtrnd_img = main.star_tracker(
        ...
        x_cat=x_cat,
        k=k,
        indexed_star_pairs=indexed_star_pairs,
        darkframe_file=darkframe_file_path,
        min_star_area=min_star_area,
        max_star_area=max_star_area,
        isa_thresh=isa_thresh,
        nmatch=nmatch,
        n_stars=max_num_stars_to_process,
        low_thresh_pxl_intensity=low_thresh_pxl_intensity,
        hi_thresh_pxl_intensity=hi_thresh_pxl_intensity,
        graphics=graphics,
        verbose=VERBOSE,
    )
...
filename = (
    now + "_data_file_nm-" + str(nmatch) + "_pxl-" + str(starMatchPixelTol) + ".csv"
)
